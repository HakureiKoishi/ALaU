...
#-----------------------定义-----------------------
    ...
    if os.path.exists(Desktop+r'\请把需要加密的文件放入此文件夹'):
        ...
        readme = '''>本程序尚处于开发测试版本，使用该程序进行加密前，请做好文件备份以防止不必要的意外损失<
    ！请勿移动任何程序生成的文件夹的路径 !
    *请勿在桌面仍保留> 加密文件容器.AZLocked <为名的文件的情况下进行本次加密，否则本次加密将无法生效*
    ...
    密钥文件夹使用说明：请把需要当做密钥的文件放入密钥文件夹（密钥文件夹内必须且只能存在一个文件，不限后缀）目前只能使用密钥文件加密，请妥善保存！'''
        file.write(readme)
        file.close()
    if os.path.exists(Desktop+r'\请把需要加密的文件放入此文件夹\请把任意文件放入此文件夹中以作为密钥文件（解锁用）'):
        pass
    else:
        os.makedirs(Desktop+r'\请把需要加密的文件放入此文件夹\请把任意文件放入此文件夹中以作为密钥文件（解锁用）')
